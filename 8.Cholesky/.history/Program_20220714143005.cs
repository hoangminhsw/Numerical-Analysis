...
/*                Matrix U = Matrix.FindU(M);
                if(U is Matrix)
                    {Console.WriteLine("==========M = AT*A:=========");
                    Matrix.output(M);
                    Console.WriteLine("==========Ma tran U va uT:=========");
                    Matrix.output(U);
                    Matrix.output(Matrix.chuyenvi(U));
                    Matrix y = Matrix.gauss_nghichdao_duoi(Matrix.chuyenvi(U),d,n);
                    Console.WriteLine("==========Ma tran Y: UT*Y=D =========");
                    Matrix.output(y);
                    x = Matrix.gauss_nghichdao_tren(U,y,n);} */
                    Console.WriteLine("==========M = AT*A:=========");
                    Matrix.output(M);
                    Console.WriteLine("==========Ma tran U va uT:=========");
     ...
     public class Matrix_C
{
    ...
    public static Matrix_C FindU(Matrix A){
        int n = Matrix.dimension(A);
           ...
           /* for(int k=1;k<n;k++){    // Tìm U[1,k]
                u[0,k] = a[0,k]/u[0,0];
                Console.WriteLine(u[0,k]);
                Console.WriteLine(a[0,k]);
                Console.WriteLine(u[0,0]);
            }
            ...
            u[n-1,n-1]= Complex.Can(a[n-1,n-1]- nn);
             for(int i=1;i<Matrix.dimension(A);i++){
                for(int j=0;j<i;j++){
                    u[i,j]= new Complex(0,0);
                }
            }
