...
class Gauss_Jordan_Algorithms:
    ...
    result = np.zeros(
        (len(matrix[0]) - 1, 1))  # Khởi tạo ma trận lưu kết quả với các giá trị ban đầu bằng 0
    ...
    def find_pivot_element(self):
        ...
        if pivot_element != 0:  # Lưu vị trí hàng và cột của phần tử giải
            ...
            print("Phan tu giai: ", round(self.matrix[self.index_row[-1]][self.index_column[-1]], 10))
            print("Vi tri: ", self.index_row[-1] + 1, self.index_column[-1] + 1)
            print()
